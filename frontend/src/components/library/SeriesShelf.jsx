...
import { Layers, BookOpen, Download, BookMarked, Loader2 } from "lucide-react";
import { BOOK_MOCKUP } from "@/components/book/media";

export const SeriesShelf = ({ series, onOpen, onExport, exporting }) => (
  <section className="mb-12" data-testid="series-section">
    ...
    <div className="space-y-6">
      {series.map((volumes, si) => {
        const rootId = volumes[0].serie_root_id || volumes[0].id;
        return (
          <motion.div
            key={volumes[0].id}
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={() => onExport(rootId, "pdf", volumes[0].titolo)}
                  disabled={exporting === `${rootId}-pdf`}
                  data-testid={`series-export-pdf-${si}`}
                  className="inline-flex items-center gap-1.5 border border-[#E7E5E4] text-[#1C1917] hover:border-[#722F37] hover:text-[#722F37] rounded-sm px-3 py-1.5 text-xs font-medium transition-colors duration-300 disabled:opacity-60"
                >
                  {exporting === `${rootId}-pdf` ? (
                ...
                <button
                  onClick={() => onExport(rootId, "epub", volumes[0].titolo)}
                  disabled={exporting === `${rootId}-epub`}
                  data-testid={`series-export-epub-${si}`}
                  className="inline-flex items-center gap-1.5 border border-[#E7E5E4] text-[#1C1917] hover:border-[#722F37] hover:text-[#722F37] rounded-sm px-3 py-1.5 text-xs font-medium transition-colors duration-300 disabled:opacity-60"
                >
                  {exporting === `${rootId}-epub` ? (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  ) : (
